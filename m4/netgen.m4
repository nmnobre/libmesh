  ...
  AS_IF([test "x$enablenetgen" = "xyes"],
        [
          ...
          AS_IF([(cd contrib/netgen/build && \
                   cmake -DUSE_NATIVE_ARCH=OFF -DUSE_PYTHON=OFF -DUSE_GUI=OFF -DUSE_OCC=OFF -DCMAKE_CXX_EXTENSIONS=OFF -DCMAKE_CXX_FLAGS="$NETGEN_CXXFLAGS" -DCMAKE_INSTALL_PREFIX:PATH=$my_top_builddir/contrib/netgen/install \
